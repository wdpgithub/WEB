...
router.post('/AdminLoginAndRegHandler',function(req,res){
  if(req.query.action=='add'){
    ...
    mongo('find','infor',selector,function(data){
      console.log(data);
      if(data.length==0){
        mongo('find','infor',null,function(data){
          var selector2={};
          ...
          if(selector2.userName!='false'&&selector2.trueName!='false'&&selector2.pwd!='false'&&selector2.phone!='false'){
            mongo('ins','infor',selector2,function(data){
              if(data.result.n==1){
                res.send('注册成功')
              }else{
                res.send('注册失败')
    ...
    mongo('find','infor',selector3,function(data){
    ...
    if(req.query.action=='quit'){
     ...
     if(oldPwd==req.session.user.pwd){
      var selector=[
       ...
       mongo('updateone','infor',selector,function(data){
        if(data.result.n==1){
          res.send('修改成功')
        }else{
          res.send('修改失败')
      ...
      mongo('del','infor',selector1,function(data){
        console.log(data)
        if(data.result.n==0){
          res.send('删除失败')
        }else{
          ...
          mongo('updatemany','infor',selector2,function(data){
            // console.log(data)
            if(data.result.n==0){
              res.send('删除失败')
            }else{
...
// 拓展 ******************************************************
//视频上传 ：  formidable  cnpm i formidable
...
router.post('/UploadVideo',function(req,res){
	...
	form.uploadDir='temporay'; //上传文件的路径
	// 限制：
	form.maxFieldSize=100*1024*1024; //上传文件大小
	form.maxFields=1000; //文件上传数量
	form.keepExtensions=true;//是否保留文件拓展名
//	该方法会转换请求中所包含的表单数据，callback会包含所有字段域和文件信息
	form.parse(req,function(err,fields,files){
	//   fields 文本域    	    files 文本信息
	console.log(fields);
	console.log(files);
