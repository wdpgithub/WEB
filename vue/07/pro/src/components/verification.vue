<template>
	<div>
  <canvas class="verCanvas" width="100%" height="30px" @click="changeVeri"></canvas>
...
//      this.drawPic("ddda") // 使用绘制验证码的方法
    ...
    methods: { // 方法
     ...
     this.$axios({   
        	methods:'get',
//      	****地址是需要进行修改的
        	url:"/vueHandler/shopht?action=getVeri",
        }).then(function(response){ //200
        	console.log(response)
//      	*****这个判断是不是也要进行修改
        	if(response.data){ //获取验证码成功
        		console.log(response)
//      	*****response.data视情况进行修改
        		fn(response.data)
        	}else{
        		alert('获取验证码失败')
        	}
      ...
      drawPic: function (returnTxt) { // 把验证码画成canvas的函数
        /**绘制验证码图片**/
        /**绘制背景色**/
